    ...
    <div class="row">
      <div class="col-md-8">
            ...
            <div class="property-tab">
              ...
              <div class="tab-content">
                <div role="tabpanel" class="tab-pane active" id="description">    
                  <h3 class="text-uppercase  bottom20 top10">Application <span style="color:rgb(248, 157, 10)">Status</span></h3>                  <p class="p-font-15 top30 bottom30">Most of the application take 1-2 hours on working days </p>
       ...
       if($con->connect_error){
        ...
        $sql=" select Type,Reason,DateStart,DateEnd,DateApply,Days,Status,StaffId,FileName,CutiId from cuti WHERE StaffId='".$_SESSION['staffid']."' AND CutiId=(SELECT Max(CutiId) from cuti WHERE StaffId='".$_SESSION['staffid']."') ORDER BY DateApply DESC LIMIT 1";
           $res=$con->query($sql);
...
<span style="margin-left:20px;background:rgb(248, 157, 10);color:white;padding:5px;">Apply on : <?php echo $datecur?> </span>
...
<p class="pull-left" style="margin-top:6px;margin-bottom:10px"><i class="icon-clipboard"></i> Reason: <?php echo $row['Reason']?></div> 
                     ...
                     <?php  if($status!='Rejected'){ ?>   <a href="update.php?id=<?php echo $row["CutiId"]; ?>" class="btn_fill" name="button">Update</a> <?php } ?>
             </div>
                  </div>
                <?php }}
               else{ ?>
...
<p class="pull-right" style="margin-right:20px">Total days:<?php echo $days?></div> 
      ...
      <!--  <a style="margin-top:5px"href="upload/<?php // echo $row['FileName'];//?>"download class="btn_fill">Download</a> --><?php  //} // 
       ...
       if($status!='Rejected'){ ?>   <a href="update.php?id=<?php echo $row["CutiId"]; ?>" class="btn_fill" name="button">Update</a> <?php } ?> <?php if($status=='Pending'){ ?>
           <a href="delete.php?id=<?php echo $row["CutiId"]; ?>" class="btn_fill"name="button">Delete</a> <?php }?>
